  ...
  "tiles": [
    {
      ...
      "params": { "hostname": "127.0.0.1", "port": {{appPort}} }
    }
  ]
}</code></pre>
          </template>
          ...
          <template v-if="currentStep === 3">
            ...
            <p class="note">
              <strong>When did I need to restart?</strong> <br>
              You only need to restart the Core (the <code>monitoror</code> binary) each time you update the <code>.env</code> file.
            </p>
            <p>
              As you have updated the <code>.env</code> file, you need to stop and start again the <code>monitoror</code> command, and you should see that in start logs:
            </p>
            <pre><code>
AVAILABLE NAMED CONFIGURATIONS
  ...
  default -> ./config.json
          ...
          <div class="c-monitoror-welcome--nav">
            <button @click="currentStep -= 1" class="c-monitoror-welcome--button c-monitoror-welcome--button__back" v-if="currentStep > 1">
            ...
            <button @click="currentStep += 1" class="c-monitoror-welcome--button c-monitoror-welcome--button__plain" v-if="currentStep < 3">
              Next step
              <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M31.2 14L18.7 1.6c-1.1-1.1-3-1.1-4.1 0-1.1 1.1-1.1 2.9 0 4.1L25 16 14.6 26.3c-1.1 1.1-1.1 2.9 0 4.1 1.1 1.1 3 1.1 4.1 0L31.2 18c.6-.6.9-1.3.8-2 0-.7-.3-1.5-.8-2z"
...
<style lang="scss">
  ...
  .c-monitoror-welcome--next-step-content {
    ...
    p {
      ...
      &.note {
        background: linear-gradient(to right, var(--color-cello), rgba(32, 66, 99, 0.3));
        border-radius: 4px;
        padding: 20px 25px;
        margin-bottom: 20px;
